...
from app.modules.promotion.campaigns import create_campaign as create_owned_campaign
...
def _serialize_link(
    ...
    return serialize_link(
        ...
        offer_allowed_traffic=offer_allowed_traffic,
        offer_forbidden_traffic=offer_forbidden_traffic,
        stats=stats,
    )
...
async def create_link(
    ...
    if data.campaign_id is not None:
        campaign_id = await resolve_campaign_for_owner(
            db,
        ...
        campaign = await create_owned_campaign(
            db,
            offer=offer,
            name=link_name,
            partner_id=profile.id,
...
async def update_link(
    ...
    if "campaign_id" in update_data:
        update_data["campaign_id"] = await resolve_campaign_for_owner(
            db,
