...
async def test_create_tracking_link_creates_general_campaign(client: AsyncClient):
    ...
    campaigns = await partner.get("/api/v1/partner/campaigns")
...
async def test_campaign_lifecycle_for_tracking_links(client: AsyncClient):
    ...
    campaign = await partner.post("/api/v1/partner/campaigns", json={
        "offer_id": offer_id,
        "name": "Telegram",
        "description": "Канал",
    })
    ...
    archived = await partner.patch(f"/api/v1/partner/campaigns/{campaign_id}", json={"status": "ARCHIVED"})
    ...
    restored = await partner.patch(f"/api/v1/partner/campaigns/{campaign_id}", json={"status": "ACTIVE"})
    assert restored.status_code == 200

    listed = await partner.get(f"/api/v1/partner/campaigns?offer_id={offer_id}&status=ACTIVE")
